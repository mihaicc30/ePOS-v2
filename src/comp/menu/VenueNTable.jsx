  ...
  if (!venueNtable.venue)
    return (
      <div className="absolute inset-0 bg-[--c60] z-10 overflow-y-scroll flex flex-col items-center">
        {/* location */}
        <div className={`animate-fadeUP1 flex flex-col max-w-[400px] transition-all w-[100%] ${venueNtable.venue ? "opacity-0 absolute" : "z-20"}`}>
          <p className="text-center font-bold text-xl mt-4">Select your location</p>
          <div className="relative w-[100%]">
            <input ref={searchInputRef} name="venuesearchinput" type="text" placeholder="Search..." className="w-[100%] mx-auto pl-10 pr-10 py-2 my-2 rounded" value={searchLocationValue} onChange={handleLocationInputChange} />
            <span className="absolute top-[28px] left-2 -translate-y-3">🔍</span>
            <button
              onClick={() => setLocationSearchValue("")}
              className={`absolute top-[28px] right-5 -translate-y-3 ${searchLocationValue ? "" : "hidden"}
									`}>
              ✖
            </button>
          </div>
          ...
          {venues
            .filter((venue) => venue.name.toLowerCase().indexOf(searchLocationValue.toLowerCase()) >= 0 || venue.address.toLowerCase().indexOf(searchLocationValue.toLowerCase()) >= 0)
            .map((venue, index) => {
              return (
                <button disabled={venue.id !== 101010} onClick={() => handleLocation(venue.id)} key={index} className={` my-2 w-[100%] bg-[--c30] rounded px-3 py-2 border-b-2 border-b-[--c2] text-[--c2] relative inline-block shadow-xl active:shadow-black active:shadow-inner disabled:bg-[#cecdcd] disabled:text-[#ffffff] disabled:active:shadow-none`}>
                  <div className="flex flex-nowrap">
                    <p className={`${venue.id !== 101010 ? "text-gray-400" : ""} font-bold text-lg truncate w-[100%]`}>{venue.name}</p>
                  ...
                  <span className={`${venue.id !== 101010 ? "text-gray-400" : ""} text-sm`}>{venue.address}</span>
                </button>
              );
            })}
        </div>
  ...
  if (!venueNtable.table && venueNtable.venue)
    return (
      <div className="absolute inset-0 bg-[--c60] z-10 overflow-y-scroll flex flex-col items-center">
        ...
        <div className={`flex flex-col max-w-[400px] transition-all ${!venueNtable.table && !venueNtable.venue ? "opacity-0 absolute" : "animate-fadeUP1 z-10"} `}>
          <p className="text-center font-bold text-xl mt-4">Select your table</p>
          ...
          {getVenueById(venues, venueNtable.venue)
