            ...
            tb.layout.map((table, index) => {
              if (table.area === showArea) {
                if (table.type === "wall") {
                  ...
                  return (
                    <div  onClick={() => handleTransfer(table.tn)} key={index + "tL"} className={`relative flex flex-col rounded-xl justify-center mphism ${tableClock["t" + table.tn] ? "bg-none bg-orange-200 " : " bg-gray-200 "} items-center  `} style={{ gridColumnStart: `${table.x1}`, gridColumnEnd: `${table.x2}`, gridRowStart: `${table.y1}`, gridRowEnd: `${table.y2}` }}>
